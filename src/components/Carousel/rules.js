  ...
  return (
    <div className="w-full py-8">
      ...
      <div className="relative w-full max-w-4xl mx-auto">
        <button
          onClick={prevSlide}
          className="absolute left-0 top-1/2 -translate-x-1/2 ml-[-20rem] -translate-y-1/2 z-10 bg-white shadow hover:bg-blue-50 p-3 rounded-full transition border border-blue-100"
          aria-label="Previous"
        >
        ...
        <button
          onClick={nextSlide}
          className="absolute right-0 top-1/2 translate-x-1/2 mr-[-20rem] -translate-y-1/2 z-10 bg-white shadow hover:bg-blue-50 p-3 rounded-full transition border border-blue-100"
          aria-label="Next"
        >
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {visibleRules.map((rule, index) => (
              <div
                key={index} 
                className="bg-white rounded-xl shadow-md px-8 py-8 flex flex-col items-center border border-blue-500"
              >
            ...
            {Array.from({length: Math.ceil(rules.length / 2)},(_, i)=>(
              <div
                ...
                className={`w-3 h-3 rounded-full transition-all duration-200 ${
                  Math.floor(currentIndex / 2) === i ? 'bg-blue-600 scale-110' : 'bg-gray-300'
                }`}
              />
            ))}
          </div>
